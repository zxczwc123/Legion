...
{
    public static bool RaycastTower(List<LegionTower> towers, out LegionTower tower)
    {
        var mouseRay = GUIPointToWorldRayPrecise(Event.current.mousePosition);
    ...
    public static bool RaycastAllPathLinkLine(List<LegionTower> towers, out LegionTower start, out LegionTower end)
    {
        var mouseRay = GUIPointToWorldRayPrecise(Event.current.mousePosition);
        start = null;
        end = null;
        for (var i = 0; i < towers.Count; i++)
        {
            ...
            for (var j = 0; j < tower.towerInfo.Links.Count; j++)
            {
                ...
                var distance = MathTool.DisOfLine(tower.towerInfo.position,
                    linkPoint.towerInfo.position - tower.towerInfo.position, mouseRay.origin,
                    mouseRay.direction);
                Debug.Log(distance);
    ...
    public static Ray GUIPointToWorldRayPrecise(Vector2 guiPoint, float startZ = float.NegativeInfinity)
    {
        ...
        if (float.IsNegativeInfinity(startZ)) startZ = camera.nearClipPlane;

        var screenPixelPos = HandleUtility.GUIPointToScreenPixelCoordinate(guiPoint);
        var viewport = camera.pixelRect;

        var camToWorld = camera.cameraToWorldMatrix;
        var camToClip = camera.projectionMatrix;
        var clipToCam = camToClip.inverse;
        ...
        var rayPointClipSpace = new Vector3(
        ...
        var rayPointCameraSpace = clipToCam.MultiplyPoint(rayPointClipSpace);

        if (camera.orthographic)
        {
            // ray direction is always 'camera forward' in orthographic mode
            var rayDirectionCameraSpace = new Vector3(0.0f, 0.0f, -1.0f);
            ...
            var rayOriginCameraSpace = rayPointCameraSpace;
        ...
        else
        {
            ...
            var rayDirectionCameraSpace = rayPointCameraSpace;
            ...
            var cameraPositionWorldSpace = camToWorld.MultiplyPoint(Vector3.zero);
            var originOffsetWorldSpace = rayDirectionWorldSpace * Mathf.Abs(startZ / rayDirectionCameraSpace.z);
